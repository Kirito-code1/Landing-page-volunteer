    ...
    <div className="flex min-h-screen items-center justify-center bg-[linear-gradient(180deg,_#fffaf1_0%,_#ffffff_50%,_#eff6ff_100%)] px-4">
      <div className="w-full max-w-xl rounded-[34px] border border-gray-100 bg-white p-10 text-center shadow-xl">
        ...
        <h1 className="text-3xl font-black uppercase italic tracking-tighter text-gray-900">
          {pick({ ru: "Оплата Premium отменена", en: "Premium payment cancelled", uz: "Premium to'lovi bekor qilindi" })}
        </h1>
        <p className="mt-4 text-sm font-semibold leading-7 text-gray-500">
          {pick({
            ru: "Uzum Checkout вернул вас без подтверждённого платежа. Вы можете вернуться и снова пройти оплату, когда будете готовы.",
            en: "Uzum Checkout returned you without a confirmed payment. You can come back and try the payment again whenever you are ready.",
            uz: "Uzum Checkout sizni tasdiqlangan to'lovsiz qaytardi. Tayyor bo'lganingizda qaytib, to'lovni yana urinib ko'rishingiz mumkin.",
          })}
        </p>
        {orderId ? (
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
          <Link
            href="/premium"
            className="inline-flex items-center justify-center rounded-2xl bg-amber-500 px-8 py-4 text-sm font-black uppercase tracking-widest text-white transition-colors hover:bg-amber-600"
          >
...
function PremiumCancelFallback() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-[linear-gradient(180deg,_#fffaf1_0%,_#ffffff_50%,_#eff6ff_100%)] px-4">
...
export default function PremiumCancelPage() {
  return (
    <Suspense fallback={<PremiumCancelFallback />}>
      <PremiumCancelContent />
    </Suspense>
  );
}
